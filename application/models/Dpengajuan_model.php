...
class Dpengajuan_model extends CI_Model
{
    public function getPengajuanTemp($waktu, $divisi, $minggu)
    {
        ...
        where status='proses' AND waktu_pengajuan = '$waktu' AND user_id = '$divisi' AND minggu = '$minggu'
                    ";
    ...
    public function getDivisiTemp()
    {
        $data['user'] = $this->db->get_where('user', ['username' => $this->session->userdata('username')])->row_array();
        $user_id = $this->session->userdata('id');
        $query = "SELECT distinct(divisi), user_id, minggu, waktu_pengajuan FROM pengajuan WHERE status='proses'
                    ";
        return $this->db->query($query)->result_array();
    }
    ...
    public function getLaporan()
    {
        $data['user'] = $this->db->get_where('user', ['username' => $this->session->userdata('username')])->row_array();
        $user_id = $this->session->userdata('id');
        $query = "SELECT distinct(divisi), user_id,  minggu, waktu_pengajuan FROM pengajuan WHERE status='disetujui'
                    ";
        return $this->db->query($query)->result_array();
    }
    ...
    public function getLaporanTemp($waktu, $divisi, $minggu)
    {
        ...
        FROM pengajuan
        INNER JOIN barang ON pengajuan.barang_id=barang.id
        where status='disetujui' AND waktu_pengajuan = '$waktu' AND user_id = '$divisi' AND minggu = '$minggu'
                    ";
        return $this->db->query($query)->result_array();
    }
}