...
class Divpengajuan_model extends CI_Model
{
    public function getDivpengajuanTemp()
    {
        ...
        where user_id = $user_id AND validasi = 'belum diterima' AND status !='ditolak' AND status !='ditolak upb'
                    ";
    ...
    public function getRekap()
    {
        $data['user'] = $this->db->get_where('user', ['username' => $this->session->userdata('username')])->row_array();
        $user_id = $this->session->userdata('id');
        $query = "SELECT distinct(waktu_pengajuan), minggu FROM pengajuan WHERE status!='proses' AND user_id = $user_id
                    ";
        return $this->db->query($query)->result_array();
    }
    ...
    public function getRekapDetail($waktu, $minggu)
    {
        $data['user'] = $this->db->get_where('user', ['username' => $this->session->userdata('username')])->row_array();
        $user_id = $this->session->userdata('id');
        $query = "SELECT barang.nama_brg, pengajuan.minggu, pengajuan.id, pengajuan.waktu_pengajuan, pengajuan.realisasi , pengajuan.jumlah, pengajuan.satuan, pengajuan.harga, pengajuan.total, pengajuan.status, pengajuan.waktu_validasi, pengajuan.validasi, pengajuan.user_id, pengajuan.keterangan
        FROM pengajuan
        INNER JOIN barang ON pengajuan.barang_id=barang.id
        where status !='proses' AND waktu_pengajuan = '$waktu' AND minggu = '$minggu' AND user_id = $user_id
                    ";
        return $this->db->query($query)->result_array();
    }
