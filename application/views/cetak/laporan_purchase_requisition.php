...
<body>
    ...
    $romawi = romawi($bulan);
    $taun = date('Y');
    $query = $this->db->query("SELECT max(kode_surat) as maxKode FROM surat where month(tanggal_surat) = '$bulan' AND year(tanggal_surat) = '$taun' ");
    foreach ($query->result() as $surats) {
    ?>
    <p align="center" style="font-weight: bold; font-size: 15px;"><?= $surats->maxKode ?></p>
    <?php } ?> <br>
    ...
    ?>
            Unit Yang Mengajukan : <b><?= $users->user ?></b>
            </b>
        </p>
        <?php } ?>
        ...
        <table class="tabel2">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td style="text-align: left; font-size: 12px;"><?= $dpr['deskripsi']; ?></td>
            ...
            <tfoot>
                ...
                $totalp = $this->db->query("SELECT sum(total) as totalp FROM pengajuan Where minggu = $minggu AND waktu_pengajuan = '$tanggal' AND user_id = '$user_id' AND status = 'Diusulkan keuangan'");
